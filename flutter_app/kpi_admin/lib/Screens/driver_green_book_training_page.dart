// lib/Screens/driver_green_book_training_page.dart
//
// DA Academy — Green-Book-Schulung (Fahrtenkontrollbuch /
// Tageskontrollblätter nach § 1 Abs. 6 FPersV).
//
// Ablauf: Intro → Kapitelübersicht → Kapitel-Slides → Abschlusstest
// (alle Fragen, 80 % zum Bestehen) → Ergebnis.
//
// Bewusst schlanker als das Fahrsicherheitstraining: kein Modul-Quiz
// und kein Zertifikat-PDF — nur ein Abschlusstest am Ende.

import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';
import 'package:intl/intl.dart';

import '../data/safety_training/green_book_data.dart';
import '../data/safety_training/green_book_texts.dart';
...
const String kGreenBookTestId = 'green_book';
const double kGreenBookPassThreshold = 0.8;

class DriverGreenBookTrainingPage extends StatefulWidget {
  final String dspUid;
  final String driverTransporterId;
  final VoidCallback onBack;

  const DriverGreenBookTrainingPage({
    super.key,
  ...
  @override
  State<DriverGreenBookTrainingPage> createState() =>
      _DriverGreenBookTrainingPageState();
}

enum _Step { intro, chapters, slides, exam, done }

class _DriverGreenBookTrainingPageState
    extends State<DriverGreenBookTrainingPage> {
  ...
  List<SafetyChapterContent> get _chapters =>
      greenBookChaptersFor(_lang);

  List<DrivingQuestion> get _questions => greenBookQuestionsFor(_lang);
  ...
  String _t(String key, [Map<String, String>? vars]) =>
      greenBookText(_lang, key, vars: vars);

  /// Bestehensgrenze als ganze Prozentzahl, z. B. „80".
  String get _threshold => (kGreenBookPassThreshold * 100).round().toString();
  ...
  Future<void> _loadProgress() async {
    try {
      ...
      setState(() {
        ...
        if (score != null && score >= kGreenBookPassThreshold) {
          _passedScore = score;
        }
    ...
    return ListView(
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: passed ? _kMint : const Color(0xFFF3F4F6),
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
                color: read ? _kGreen.withValues(alpha: 0.45) : _kLine,
              ),
            ),
            child: Row(
              children: [
  ...
  Widget _buildExam() {
    ...
    final passed = score >= kGreenBookPassThreshold;
